...
fn root_file_template(root_file: &str, agent_name: &str) -> String {
    match root_file {
        "CLAUDE.md" => format!(
            "# Project AI Configuration\n\n此文件由 AgentBuddy 生成，用于 Claude Code 项目级配置。\n\n> Agent: {}\n",
            agent_name
        ),
        "GEMINI.md" => format!(
            "# Project AI Configuration\n\n此文件由 AgentBuddy 生成，用于 Gemini CLI 项目级配置。\n\n> Agent: {}\n",
            agent_name
        ),
        _ => format!(
            "# Project AI Configuration\n\n此文件由 AgentBuddy 生成，用于 AI coding agent 项目级配置。\n\n> Agent: {}\n",
            agent_name
        ),
    }
}

fn symlink_root_file_content() -> &'static str {
    "<!-- Generated by AgentBuddy -->\nRead `.agents/AGENTS.md` for full project guidance before any work.\nShared commands / rules / skills / agents live under `.agents/`.\n"
}

fn agents_md_content() -> &'static str {
    "# Project Agents Guide\n\n\
此文件由 AgentBuddy 在「软链接模式」下生成，作为项目级共享约定入口。\n\n\
## 目录约定\n\n\
| 路径 | 用途 |\n\
|------|------|\n\
| `.agents/commands` | 共享斜杠命令 / 工作流 |\n\
| `.agents/rules` | 共享规则与约束 |\n\
| `.agents/skills` | 共享 Skills |\n\
| `.agents/agents` | 共享子 agent 定义 |\n\n\
各 agent 配置目录（如 `.claude/`、`.codex/`）下的同名子目录通过软链接指向此处，只需维护一份内容。\n\n\
## 项目约定\n\n\
在此补充仓库级说明（构建、测试、代码风格、禁止事项等）。\n"
}
...
pub fn init_project_config(
    ...
    match mode {
        InitMode::Full => {
            let mut seen_config_dirs: HashSet<String> = HashSet::new();
            // Track which root files have been written (e.g. AGENTS.md shared by multiple agents)
            ...
            for req in selected_agents {
                ...
                if let Some(rf) = spec.root_file {
                    if written_root_files.insert(rf.to_string()) {
                        let p = base.join(rf);
                        let content = root_file_template(rf, spec.name);
                        write_file(
                            &p,
                            &content,
                            overwrite,
        ...
        InitMode::Symlink => {
            ...
            for req in selected_agents {
                ...
                // root file → short pointer to shared guide
                if let Some(rf) = spec.root_file {
                    if written_root_files.insert(rf.to_string()) {
                        let p = base.join(rf);
                        write_file(
                            &p,
                            symlink_root_file_content(),
                            overwrite,
                            &mut created,
                            &mut skipped,
                            &mut errors,
...
mod tests {
    ...
    #[test]
    fn full_mode_dedupes_shared_root_and_codebuddy_dir() {
        ...
        // First writer among shared AGENTS.md agents is codebuddy
        assert!(agents_md.contains("codebuddy"), "{agents_md}");
    ...
    #[test]
    fn check_reports_existing_and_dedupes_codebuddy() {
        let base = scratch("check");
        fs::write(base.join("AGENTS.md"), "x\n").unwrap();
        fs::create_dir_all(base.join(".codebuddy")).unwrap();
        let result = check_project_config_exists(
            base.to_str().unwrap(),
            &[req("codebuddy"), req("codebuddy-cn"), req("codex")],
            &InitMode::Full,
        )
        .unwrap();
        ...
        assert_eq!(
            paths
                .iter()
                .filter(|p| p.ends_with(".codebuddy"))
                .count(),
            1
        );
        cleanup(&base);
    ...
    fn symlink_mode_creates_relative_dir_links() {
        ...
        let agents_md = fs::read_to_string(base.join(".agents/AGENTS.md")).unwrap();
        assert!(agents_md.contains("Project Agents Guide"), "{agents_md}");
        ...
        // Shared AGENTS.md root once; CLAUDE.md pointer for claude-code
        assert!(base.join("AGENTS.md").is_file());
