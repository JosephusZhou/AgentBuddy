...
import type { CodexEnvironment, CodexEnvCandidate, CodexEnvShellStatus } from "./codex-env/types";
      ...
      void invokeProviderList()
        .then(async (rows) => {
          ...
          setCloneProviders([
            ...
            ...(routeAgg ? [routeAgg, ...eligible] : eligible),
          ]);
        })
  ...
  const openEdit = useCallback(async (env: CodexEnvironment) => {
    ...
    void invokeProviderList()
      .then(async (rows) => {
        ...
        const linked =
          [OFFICIAL_OAUTH_PROVIDER, ...all].find((p) => p.id === env.providerId) ?? null;
    ...
    <>
        ...
        <div className="content-header-bar">
          ...
          <div className="header-actions">
            ...
            <button
              ...
              data-tooltip={loaded && envs.length === 0 ? "请先安装 Codex" : "扫描已有目录"}
            ...
            <button
              ...
              data-tooltip={loaded && envs.length === 0 ? "请先安装 Codex" : "从现有环境复制"}
              onClick={() => openClone()}
        ...
        {!loaded ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-state-subtext">
              请先安装 Codex CLI（例如 <code>codex</code>{" "}
            ...
            {envs.map((env) => (
              ...
              >
                <div className="claude-env-card-main">
                  ...
                  <div className="claude-env-meta">
                    启动：
                    {env.isDefault ? <code>codex</code> : <code>{env.aliasName}</code>}
                    {!env.isDefault && (
                      <span className="claude-env-meta-dim"> （CODEX_HOME → 该目录）</span>
                    )}
                  </div>
                  ...
                  {(env.model || env.modelProvider || env.baseUrl) && (
                    <div className="claude-env-meta">
                      {env.model ? (
                        <>
                          模型：<code>{env.model}</code>{" "}
                        </>
                      ) : null}
                      {env.modelProvider ? (
                        <>
                          Provider：<code>{env.modelProvider}</code>{" "}
                        </>
                      ) : null}
                      {env.baseUrl ? (
                        <>
                          Base：<code>{env.baseUrl}</code>
                        </>
                      ) : null}
                    </div>
                  )}
        ...
        <div className="modal modal-lg claude-env-modal">
          ...
          <div className="modal-body claude-env-modal-body">
            <div className="form-group">
              <label className="form-label" id="xe-source-label">
                源环境
              </label>
              <div
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="xe-name">
                显示名称
              </label>
              <input
                ref={nameInputRef}
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="xe-slug">
                slug
              </label>
              <input
                id="xe-slug"
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="xe-dir">
                配置目录
              </label>
              <input
                id="xe-dir"
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="xe-alias">
                shell 别名
              </label>
              <input
                id="xe-alias"
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="xe-notes">
                备注（可选）
              </label>
              <input
                id="xe-notes"
            ...
            {cloneProviders.length > 0 && (
              <div className="form-group">
                <label className="form-label" htmlFor="xe-clone-provider">
                  选择供应商（可选）
                </label>
                <CodexProviderSelect
                    ...
                    if (p) {
                      ...
                      const defaultModel =
                        p.providerType === "openai"
                          ? p.defaultModel
                          : p.openaiDefaultModel || p.defaultModel;
            ...
            {!cloneUsesOfficialOAuth && (
              <div className="form-group">
                <label className="form-label" htmlFor="xe-base-url">
                  Base URL（可选）
                </label>
                <input
                  id="xe-base-url"
            ...
            {!cloneUsesOfficialOAuth && (
              <div className="form-group">
                <label className="form-label" htmlFor="xe-api-key">
                  Token（可选）
                </label>
            ...
            {!cloneUsesOfficialOAuth && (
              <div className="form-group">
                <div className="claude-env-model-label-row">
                  <label className="form-label" htmlFor="xe-model">
                    模型（可选）
                  </label>
                  <button
                    type="button"
            ...
            {!cloneUsesOfficialOAuth && (
              <div className="form-group">
                <label className="form-label" htmlFor="xe-provider">
                  Provider（可选）
                </label>
                <input
                  id="xe-provider"
            ...
            <div className="form-group">
              ...
              <label className="ui-check" htmlFor="xe-sync-mcp">
                ...
                <CheckGlyph />
                <span className="ui-check-label">
                  同步默认环境 MCP（将 <code>~/.codex/config.toml</code> 的 [mcp_servers]
                  写入新环境）
                </span>
              </label>
            ...
            <div className="claude-env-form-hint">
              <span className="claude-env-form-hint-line">
                * 始终复制 config.toml；AGENTS.md、skills/
                按上方勾选决定是否复制。其他数据默认不复制；勾选后会复制源环境中除上述选项外的目录和文件。
              </span>
              <span className="claude-env-form-hint-line">
                * model / provider / Base URL 留空时沿用源环境 config.toml；填写后写入目标
                config.toml。
              </span>
              <span className="claude-env-form-hint-line">
                * Token 留空不写 auth.json；填写后写入目标环境 <code>auth.json</code> 的{" "}
              ...
              <span className="claude-env-form-hint-line">
                * 软链接同步会使已选的 skills、AGENTS.md
                与其他数据跟随源环境变化；全量同步会创建独立副本。
              </span>
              <span className="claude-env-form-hint-line">
                * 由于 Codex MCP 与供应商配置共同位于 <code>config.toml</code>，软链接模式无法对 MCP
                单独建立双向同步，Codex MCP 仍会按配置内容复制到新环境。
              </span>
              <span className="claude-env-form-hint-line">
      ...
      <div className={`modal-overlay ${showEdit ? "visible" : ""}`} {...editDismiss}>
        <div className="modal claude-env-modal">
          ...
          <div className="modal-body claude-env-modal-body">
            <div className="form-group">
              <label className="form-label" htmlFor="xe-edit-name">
                显示名称
              </label>
              <input
                ref={nameInputRef}
            ...
            {!editIsDefault && (
              <>
                <div className="form-group">
                  <label className="form-label" htmlFor="xe-edit-slug">
                    slug
                  </label>
                  <input
                    id="xe-edit-slug"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="xe-edit-dir">
                    配置目录
                  </label>
                  <input
                    id="xe-edit-dir"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="xe-edit-alias">
                    shell 别名
                  </label>
                  <input
                    id="xe-edit-alias"
            ...
            {editProviders.length > 0 && (
              <div className="form-group">
                <label className="form-label" htmlFor="xe-edit-provider">
                  选择供应商（可选）
                </label>
                <CodexProviderSelect
                  id="xe-edit-provider"
            ...
            {!editUsesOfficialOAuth && (
              <div className="form-group">
                <label className="form-label" htmlFor="xe-edit-base-url">
                  Base URL
                </label>
                <input
                  id="xe-edit-base-url"
            ...
            {!editUsesOfficialOAuth && (
              <div className="form-group">
                <label className="form-label" htmlFor="xe-edit-api-key">
                  Token
                </label>
                <div className="form-input-with-action">
                  <input
            ...
            {!editUsesOfficialOAuth && (
              <div className="form-group">
                <div className="claude-env-model-label-row">
                  <label className="form-label" htmlFor="xe-edit-model">
                    模型
                  </label>
                  <button
                    type="button"
            ...
            {!editUsesOfficialOAuth && (
              <div className="form-group">
                ...
                />
                <div className="claude-env-form-hint">
                  模型 / Provider / Base URL 预填 config.toml 当前值，留空并保存即删除对应键。 Token
                  读写 <code>auth.json</code> 的 <code>OPENAI_API_KEY</code>
            ...
            {editIsDefault && (
              <div className="claude-env-form-hint">
                默认环境路径固定为 <code>~/.codex</code>，直接运行 <code>codex</code>{" "}
                使用；该配置将写入默认 config.toml / auth.json。
              </div>
            )}
            <div className="form-group">
              <label className="form-label" htmlFor="xe-edit-notes">
                备注
              </label>
              <input
                id="xe-edit-notes"
        ...
        <div className="modal" style={{ width: 440 }}>
          ...
          <div className="confirm-body">
            ...
            <div className="confirm-subtext">
              ...
              别名会自动指向新路径。建议先关闭正在使用该环境的 Codex 进程。
            </div>
          </div>
          <div className="modal-footer">
            <button
